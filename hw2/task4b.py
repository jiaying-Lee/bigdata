...
c1 = line1.map(lambda x: (x[18],(x[5],float(x[8])/float(x[5]) if float(x[5]) != 0 else 0,1))).reduceByKey(lambda x,y : (float(x[0])+float(y[0]),float(x[1])+float(y[1]),float(x[2])+float(y[2])))
